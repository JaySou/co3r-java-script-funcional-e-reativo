const fs = require('fs');
const path = require('path')

function lerDiretorio(caminho) {

    return new Promise((resolve, reject) => {

        try {

            const arquivosCompletos = fs.readdirSync(caminho)
                                            .map(arquivo => path.join(caminho, arquivo))
            resolve(arquivosCompletos)

        } catch (e) {

            reject(e)

        }
    })
}

function lerArquivo(caminho) {
    return new Promise((resolve, reject) => {

        try {

            const conteudo = fs.readFileSync(caminho, { encoding: 'utf-8' })
            resolve(conteudo.toString())

        } catch (e) {

            reject(e)

        }
    })
}

function lerArquivos(caminhos){
    return Promise.all(caminhos.map(caminho => lerArquivo(caminho)))
}

function pegaElementosTerminadosCom(terminadoCom){
    return function(array) {
        return array.filter(el => el.endsWith(terminadoCom))
    }
}

function removeElementosSeVazio(array){
    return array.filter(el => el.trim())
}

function removerElementosSeTemTexto(texto){

    return function(array) {

        return array.filter(el => !el.includes(texto))

    }
}

function removerElementosSeEhNumero(array){
    return array.filter(el => {
        const num = parseInt(el.trim())
        return num !== num
    })
}

function removerSimbolosDoElemento(simbolos){
    return function(array){
        return array.map(el => {

            return simbolos.reduce((acc, simbolo) => {
                return acc.split(simbolo).join('')
            }, el)
        })
    }
}

function mesclarElementos(array) {

    return array.join(' ')
}

function separarTextoPorSimbolo(simbolo){
    return function(texto){
        return texto.split(simbolo)
    }
}

function agruparElementos(array){
    return Object.values(array.reduce((agrupamento, item) => {
        const el = item.toLowerCase()
        const qtde = agrupamento[el] ? agrupamento[el].qtde + 1 : 1
        agrupamento[el] = { elemento: el, qtde }

        return agrupamento

    }, {}))
}

function ordenarPorAtributo(attr, ordem = 'asc'){
    return function(array){
        const asc = (o1, o2) => o1[attr] - o2[attr]
        const desc = (o1, o2) => o2[attr] - o1[attr]
        return [...array].sort(ordem === 'asc' ? asc : desc )
    }
}


function composicao(...fns){
    return function(valor) {
        return fns.reduce(async (acc, fn) => { 
            if(Promise.resolve(acc) === acc){
                return  fn(await acc)
            }else {
                return  fn(acc)
            }
        }, valor)
    }
}

module.exports = {
    lerDiretorio,
    pegaElementosTerminadosCom,
    lerArquivo,
    lerArquivos,
    removeElementosSeVazio,
    removerElementosSeTemTexto,
    removerElementosSeEhNumero,
    removerSimbolosDoElemento,
    mesclarElementos,
    separarTextoPorSimbolo,
    agruparElementos,
    ordenarPorAtributo,
    composicao
}